/**
 * Main JavaScript for Identity Theft and Detectives interactive lesson
 */
...
function startModuleNarration(moduleId) {
    ...
    switch(moduleId) {
        case 'intro':
            if (window.narrator) {
                ...
                if (introModule) {
                    ...
                    let introContent = "Welcome to 'Identity Theft and Detectives'. ";
                    ...
                    window.narrator.speak("Welcome to the interactive English lesson on 'Identity Theft and Detectives'. This lesson explores the world of detectives, private investigators, and digital identity protection.");
                }
            }
            break;
        ...
        case 'prereading':
            if (window.narrator) {
                ...
                if (prereadingModule) {
                    ...
                    // Fallback if module not found
                    window.narrator.speak("Welcome to the Let's Begin section. Here we'll discuss identity theft in the digital age and the importance of protecting personal information online.");
                }
            }
            break;
        ...
        case 'thinking-text':
            if (window.narrator) {
                window.narrator.speak("Welcome to the Reading Comprehension Questions section. Here you'll analyze the detective interview and answer questions about it.");
            }
            break;
            
        case 'thinking-language':
            if (window.narrator) {
                window.narrator.speak("Welcome to the Vocabulary and Grammar section. Here you'll explore vocabulary, grammar exercises, and language skills from the texts.");
            }
            break;
            
        case 'activities':
            if (window.narrator) {
                window.narrator.speak("Welcome to the Activities section. Here you'll practice listening, speaking, and writing skills related to identity theft and detective work.");
            }
            break;
...
function getModuleName(moduleId) {
    switch (moduleId) {
        ...
        case 'thinking-text': return 'Questions';
...
function startJourney() {
    ...
    if (narrator) {
        ...
        if (prereadingModule) {
            const contentBlocks = prereadingModule.querySelectorAll('.content-block');
            let prereadingContent = "Welcome to the interactive English lesson on 'Identity Theft and Detectives'. Let's begin by exploring identity theft in the digital age and the importance of protecting personal information online. ";
            ...
            // Fallback if module not found
            narrator.speak("Welcome to the interactive English lesson on 'Identity Theft and Detectives'. Let's begin by exploring identity theft in the digital age and the importance of protecting personal information online.");
        }
    }
}
...
// Infinitive checking function
function checkInfinitive() {
    const inputs = document.querySelectorAll('.infinitive-input');
    let correctCount = 0;
    ...
    const feedbackEl = document.getElementById('infinitiveFeedback');
    if (feedbackEl) {
        feedbackEl.textContent = `You got ${correctCount} out of ${inputs.length} correct!`;
        feedbackEl.className = 'feedback-message show';
        feedbackEl.classList.add(correctCount === inputs.length ? 'success' : 'error');
...
// MCQ Vocabulary Functions
let mcqAnswers = {}; // Store user answers

// Select MCQ answer
function selectMCQAnswer(questionNumber, answer) {
    mcqAnswers[questionNumber] = answer;
    
    // Visual feedback for selection
    const questionElement = document.querySelector(`[data-question="${questionNumber}"]`);
    if (questionElement) {
        // Remove previous selection styling
        questionElement.querySelectorAll('.mcq-option').forEach(option => {
            option.classList.remove('selected');
        });
        
        // Add selection styling to chosen option
        const selectedOption = questionElement.querySelector(`input[name="q${questionNumber}"][value="${answer}"]`);
        if (selectedOption) {
    ...
    if (window.narrator) {
        const word = getWordForQuestion(questionNumber);
        window.narrator.speak(`You selected option ${answer.toUpperCase()} for the word "${word}".`);
    }
}

// Get the word for a specific question number
function getWordForQuestion(questionNumber) {
    const words = {
        1: 'database',
        2: 'violation', 
        3: 'surveillance',
    ...
    return words[questionNumber] || 'unknown';
}

// Check MCQ answers
function checkMCQAnswers() {
    const correctAnswers = {
        1: 'a', // database: a structured set of data held in a computer
        2: 'b', // violation: the act of going against or refusing to obey a law
        3: 'c', // surveillance: close observation, especially of a suspected spy or criminal
        4: 'd', // insurance: an agreement that provides compensation for specified loss
        5: 'a', // ambiguous: open to more than one interpretation; not having one obvious meaning
        6: 'b', // forge: to make an illegal copy of something in order to deceive
        7: 'c', // sinister: suggesting evil or harm
        8: 'd'  // scandal: an action or event regarded as morally or legally wrong
    };
        ...
        if (questionElement) {
            ...
            if (userAnswer) {
                if (userAnswer === correctAnswer) {
                    correctCount++;
                    // Mark correct answer
                    const correctOption = questionElement.querySelector(`input[name="q${i}"][value="${correctAnswer}"]`);
                    if (correctOption) {
                        correctOption.closest('.mcq-option').classList.add('correct');
                    }
                } else {
                    // Mark incorrect answer and show correct one
                    const userOption = questionElement.querySelector(`input[name="q${i}"][value="${userAnswer}"]`);
                    const correctOption = questionElement.querySelector(`input[name="q${i}"][value="${correctAnswer}"]`);
                    
                    if (userOption) {
                        userOption.closest('.mcq-option').classList.add('incorrect');
                    }
                    if (correctOption) {
                        correctOption.closest('.mcq-option').classList.add('correct');
                    }
    ...
    if (feedbackEl) {
        ...
        if (correctCount === totalQuestions) {
            feedbackText = `🎉 Excellent! You got all ${totalQuestions} questions correct! (${percentage}%)`;
        } else if (correctCount >= totalQuestions * 0.8) {
            feedbackText = `👍 Good job! You got ${correctCount} out of ${totalQuestions} correct! (${percentage}%)`;
        } else if (correctCount >= totalQuestions * 0.6) {
            feedbackText = `📚 Not bad! You got ${correctCount} out of ${totalQuestions} correct! (${percentage}%) Keep practicing!`;
        } else {
            feedbackText = `📖 You got ${correctCount} out of ${totalQuestions} correct! (${percentage}%) Review the vocabulary and try again!`;
        }
        
        feedbackEl.textContent = feedbackText;
        feedbackEl.className = 'feedback-message show';
        feedbackEl.classList.add(correctCount >= totalQuestions * 0.8 ? 'success' : 'error');
    ...
    // Provide audio feedback
    if (window.narrator) {
        const word = correctCount === totalQuestions ? 'perfect' : 
                    correctCount >= totalQuestions * 0.8 ? 'excellent' : 
                    correctCount >= totalQuestions * 0.6 ? 'good' : 'needs improvement';
        
        window.narrator.speak(`You scored ${correctCount} out of ${totalQuestions} questions. That's ${word}!`);
    }
}