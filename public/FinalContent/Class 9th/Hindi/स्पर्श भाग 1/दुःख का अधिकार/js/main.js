/**
 * Main JavaScript for दुःख का अधिकार interactive Hindi lesson
 */
...
window.score = score;
window.modulesCompleted = modulesCompleted;
...
function startModuleNarration(moduleId) {
    ...
    switch(moduleId) {
        case 'intro':
            if (window.narrator) {
                // Read only the visible content from the HTML
                const introModule = document.getElementById('intro');
                if (introModule) {
                    const contentBlocks = introModule.querySelectorAll('.content-block');
                    let introContent = "दुःख का अधिकार में आपका स्वागत है। ";
                    
                    // Extract text from visible content blocks only
                    ...
                    // Speak only the visible content
                    window.narrator.speak(introContent);
                } else {
                    // Fallback if module not found
                    window.narrator.speak("दुःख का अधिकार में आपका स्वागत है।");
                }
            }
            break;
            
        // Prereading module removed - skip to story
        ...
        case 'thinking-language':
            if (window.narrator) {
                window.narrator.speak("भाषा अध्ययन खंड में आपका स्वागत है। यहां आप शब्दों के अर्थ, शब्द-युग्म और व्याकरण का अध्ययन करेंगे।");
            }
            break;
...
function startJourney() {
    ...
    showModule('story');
    if (narrator) {
        narrator.speak("दुःख का अधिकार में आपका स्वागत है। आइए कहानी से शुरू करते हैं।");
    }
}
...
function toggleReadAloud() {
    // This function will be defined in yashpal-story.js
    if (typeof readCurrentStoryPartAloud === 'function') {
        readCurrentStoryPartAloud();
    } else {
...
function saveReflection() {
    ...
    // Prereading module removed - no progress tracking needed
...
function checkSentences() {
    const sentenceAnswers = {
        sentence1: "छन्नी-ककना",
        sentence2: "दुअन्नी-चवन्नी", 
        sentence3: "फफक-फफककर",
        sentence4: "झाड़ना-फूँकना",
        sentence5: "लिपट-लिपटकर"
    };
    
    let correctCount = 0;
    let totalCount = Object.keys(sentenceAnswers).length;
    
    Object.keys(sentenceAnswers).forEach(id => {
        const textarea = document.getElementById(id);
        if (textarea && textarea.value.trim()) {
            // Check if the answer contains the key word
            const answer = textarea.value.trim().toLowerCase();
            const keyWord = sentenceAnswers[id].toLowerCase();
    ...
    if (feedbackEl) {
        if (correctCount === totalCount) {
            feedbackEl.textContent = 'सभी उत्तर सही हैं! शब्द-युग्मों का प्रयोग उत्कृष्ट है।';
            feedbackEl.className = 'feedback-message show success';
...
function saveResearchNotes() {
    ...
    // Disable the button
    const button = document.querySelector('button[onclick="saveResearchNotes()"]');
    if (button) {
...
// Reset color feedback for activities (useful for testing)
function resetActivitiesColorFeedback() {
