...
function checkAnswer(optionIndex) {
    ...
    const correctAnswers = [0, 2, 1]; // a, c, b (0-indexed)
...
function showQuizResults() {
    ...
    // Question 1: A) Kanshi Ram (value="a")
    const q1Answer = document.querySelector('input[name="q1"]:checked');
    if (q1Answer && q1Answer.value === 'a') {
        correctCount++;
    }

    // Question 2: C) Integral humanism (value="c")
    const q2Answer = document.querySelector('input[name="q2"]:checked');
    if (q2Answer && q2Answer.value === 'c') {
        correctCount++;
    }

    // Question 3: (b) A and B (value="b")
...
function checkMatchingAnswers() {
    ...
    // Define correct answers for political parties matching
    const correctAnswers = {
        '1': 'c',  // Congress Party → United Progressive Alliance
        '2': 'a',  // Bharatiya Janata Party → National Democratic Alliance
        '3': 'd',  // Communist Party of India (Marxist) → Left Front
        '4': 'b'   // Telugu Desam Party → State party
    };
