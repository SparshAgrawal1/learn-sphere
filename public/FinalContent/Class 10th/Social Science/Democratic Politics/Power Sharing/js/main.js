...
function startModuleNarration(moduleId) {
    ...
    switch(moduleId) {
        case 'intro':
            speak("Welcome to Power-sharing. This chapter explores how democracies manage power distribution through examples from Belgium and Sri Lanka, highlighting the importance of power-sharing for maintaining balance, unity, and stability in democratic systems.");
            break;
            
        case 'overview':
            speak("This chapter resumes our exploration of democracy, focusing on power-sharing as a fundamental principle. It examines how democracies distribute power among different organs of government and various social groups, using case studies from Belgium and Sri Lanka to illustrate the importance and challenges of power-sharing in diverse societies.");
            break;
            
        case 'southafrica':
            speak("This section examines the ethnic compositions and tensions in Belgium and Sri Lanka, two countries with diverse populations. Belgium has Dutch, French and German-speaking communities with complex regional distributions, while Sri Lanka has Sinhala and Tamil-speaking groups with religious differences, highlighting the challenges of managing diversity in democratic societies.");
            break;
            
        case 'whyconstitution':
            speak("This section examines majoritarianism in Sri Lanka, where Sinhala leaders established dominance through measures like making Sinhala the official language, implementing preferential policies, and constitutionally protecting Buddhism. These actions alienated the Tamil minority and led to strained relations, struggles for autonomy, and eventually a devastating civil war that ended in 2009.");
            break;
            
        case 'indianconstitution':
            speak("This section examines Belgium's innovative power-sharing arrangements that enable different linguistic communities to live together harmoniously. Through constitutional amendments and creative institutional design, Belgium has developed a model with equal representation, regional autonomy, and community governments that has successfully prevented division and conflict.");
            break;
            
        case 'values':
            speak("This section explores the various forms of power sharing in modern democracies. It begins by explaining how the concept of power sharing emerged as a challenge to undivided political power, emphasizing that democracy requires distributing power among citizens through different arrangements. The section covers four main forms: horizontal distribution of power among government organs like legislature, executive, and judiciary; vertical division of power between different levels of government; community government for different social groups; and power sharing among political parties and pressure groups. These forms ensure checks and balances, accommodate diversity, and promote democratic participation.");
            break;


        case 'activities':
            speak("Test your knowledge and understanding with these interactive activities about Power Sharing.");
            break;
...
function startJourney() {
    ...
    showAchievement('Journey Begins', 'You started your learning journey with Power Sharing!');
...
function checkQuizAnswers() {
    ...
    // Question 1: (a) A,B,D,F
    const q1Answer = document.querySelector('input[name="q1"]:checked');
    if (q1Answer && q1Answer.value === 'a') {
        score++;
        results.push('<p class="correct-answer">✓ Question 1: Correct! A, B, D, F are in favour of power sharing.</p>');
    } else {
        results.push('<p class="wrong-answer">✗ Question 1: Incorrect. The correct answer is (a) A,B,D,F.</p>');
    }

    // Question 2: (d) B, C and D
    const q2Answer = document.querySelector('input[name="q2"]:checked');
    if (q2Answer && q2Answer.value === 'd') {
        score++;
        results.push('<p class="correct-answer">✓ Question 2: Correct! B, C and D are correct statements about power sharing in Belgium and Sri Lanka.</p>');
    } else {
        results.push('<p class="wrong-answer">✗ Question 2: Incorrect. The correct answer is (d) B, C and D.</p>');
    }

    // Question 3: (b) Both A and B are true
    const q3Answer = document.querySelector('input[name="q3"]:checked');
    if (q3Answer && q3Answer.value === 'b') {
        score++;
        results.push('<p class="correct-answer">✓ Question 3: Correct! Both statements A and B are true about power sharing.</p>');
    } else {
        results.push('<p class="wrong-answer">✗ Question 3: Incorrect. The correct answer is (b) Both A and B are true.</p>');
    }
    ...
    if (score === totalQuestions) {
        setTimeout(() => {
            showAchievement('Quiz Master', 'Perfect score! You have excellent knowledge of Power Sharing.');
        }, 1000);
    } else if (score >= 1) {
        setTimeout(() => {
            showAchievement('Good Understanding', 'Well done! You have a good grasp of Power Sharing concepts.');
        }, 1000);
    }
}
...
function toggleAccommodationNarration() {
    const btn = document.getElementById('accommodationSpeakerBtn');
    if (!btn) return;
    ...
    utter.onstart = () => {
        console.log('Forms of Power Sharing speech started successfully');
        currentSpeechState = 'speaking';
        updatePlayPauseButton();
    };
...
function toggleMajoritarianismNarration() {
    const btn = document.getElementById('majoritarianismSpeakerBtn');
    if (!btn) return;
...
function toggleBelgiumNarration() {
    const btn = document.getElementById('belgiumSpeakerBtn');
    if (!btn) return;
    ...
    try {
        ...
        window.speechSynthesis.speak(utter);
        console.log('Speech utterance sent to synthesis engine');

        // Add a timeout to check if speech actually started
        setTimeout(() => {
            console.log('After 2 seconds - speaking:', window.speechSynthesis.speaking, 'pending:', window.speechSynthesis.pending);
        }, 2000);
...
function toggleIntroNarration() {
    ...
    const curatedContent = contentElement ? contentElement.textContent.trim() : "This chapter continues the exploration of democracy by focusing on the concept of power-sharing — a key feature that ensures no single organ of government holds all the power. Through the examples of Belgium and Sri Lanka, it explains how democracies manage diverse social and political demands by distributing power among different groups and institutions. Understanding power-sharing helps us appreciate its importance in maintaining balance, unity, and stability within a democratic system.";
