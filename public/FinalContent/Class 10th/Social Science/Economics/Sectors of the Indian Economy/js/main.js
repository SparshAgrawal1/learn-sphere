...
function startModuleNarration(moduleId) {
    ...
    switch(moduleId) {
        case 'intro':
            ...
            return speak("Welcome to Sectors of the Indian Economy. This chapter introduces students to the idea of classifying economic activities into various sectors, helping them connect abstract concepts with real-life situations.");

        case 'overview':
            return speak("This section explores sectors of economic activities and how different types of work can be grouped together. We look at various economic activities happening around us and learn how to classify them using important criteria.");

        case 'whyelections':
            return speak("This section compares the three sectors of economic activity. We examine how to measure the value of goods and services produced in each sector using concepts like GDP and Gross Value Added. We also explore how the importance of different sectors has changed over time in developed countries and ask whether similar patterns are emerging in India.");

        case 'system':
            return speak("This section examines the Primary, Secondary and Tertiary Sectors in India. We look at the production of goods and services in the three sectors using data from 1977-78 and 2017-18. You can see how the total production has grown over the forty years and how the sectors have changed in importance.");

        case 'democratic':
            return speak("This section examines another way of classifying activities in the economy based on employment conditions. We look at the differences between organised and unorganised sectors through examples like Kanta, who works in an office with regular salary and benefits, and Kamal, who works as a daily wage labourer without job security. We also explore how to protect workers in the unorganised sector and why this support is necessary for both economic and social development.");

        case 'judiciary':
            return speak("This section examines sectors based on ownership - public and private sectors. The public sector is owned by the government and provides essential services like railways and roads. The private sector is owned by individuals or companies and operates to earn profits. We explore why governments must spend on activities that benefit society as a whole.");

        case 'terms':
            return speak("This section summarizes the chapter's key concepts. We review economic sector classifications, employment patterns in India, organised vs unorganised sectors, and the role of public sector activities in serving society's needs.");
...
function startJourney() {
    ...
    showAchievement('Journey Begins', 'You started your learning journey with Sectors of the Indian Economy!');
    return speak("Welcome to Sectors of the Indian Economy. This chapter introduces students to the idea of classifying economic activities into various sectors, helping them connect abstract concepts with real-life situations.");
}
...
function speakCardContent(button, text) {
    ...
    if (!isAudioEnabled) {
        console.log('Audio is disabled, attempting to enable...');
        // Try to enable audio automatically
        isAudioEnabled = true;
        globalNarrationState.isEnabled = true;
        globalNarrationState.disabledByUser = false;

        console.log('Audio enabled automatically, continuing with narration...');
        showFeedback('Audio enabled automatically, starting narration...', 'success');
    }
...
function toggleCard(cardElement) {
    console.log('toggleCard called with element:', cardElement);
    console.log('Card element classList:', cardElement.classList);

    // If the element is a button, find the parent card
    let card = cardElement;
    if (cardElement.classList.contains('card-toggle-btn') || cardElement.tagName === 'BUTTON') {
        card = cardElement.closest('.feature-card');
    ...
    console.log('Feature desc found:', !!desc);
    console.log('Toggle button found:', !!toggleBtn);
    console.log('Current desc display style:', desc ? desc.style.display : 'N/A');

    if (!desc) {
        console.error('Feature description not found in card');
        return;
    }
    ...
    if (desc.style.display === 'none' || desc.style.display === '') {
        ...
        if (toggleBtn) {
            ...
            console.log('Button text changed to: See Less');
        }
        ...
        if (toggleBtn) {
            ...
            console.log('Button text changed to: See More');
        }
        card.classList.remove('expanded');
        console.log('Card collapsed successfully');

        // Stop any ongoing narration when collapsing the card
        if (window.speechSynthesis) {
...
// Prevent card toggle when clicking on buttons inside cards
document.addEventListener('DOMContentLoaded', function() {
    // Add event listeners to prevent card toggle for narration buttons and other interactive elements
    document.querySelectorAll('.feature-card .haryana-speaker-btn, .feature-card .whyelections-speaker-btn, .feature-card .parliament-speaker-btn, .feature-card .historical-speaker-btn, .feature-card .houses-speaker-btn, .feature-card .political-speaker-btn, .feature-card .council-speaker-btn, .feature-card .powers-speaker-btn, .feature-card .president-speaker-btn, .feature-card .presidential-speaker-btn, .feature-card .judiciary-speaker-btn, .feature-card .card-toggle-btn').forEach(button => {
...
function toggleDesc(button) {
    ...
    if (desc.style.display === 'block') {
        // Card is currently expanded, we're about to collapse it
...
// Toggle historical sectors narration function
function toggleHistoricalSectorsNarration() {
    console.log('toggleHistoricalSectorsNarration called');
    ...
    // Get the historical sectors speaker button
    const historicalSectorsSpeakerBtn = document.getElementById('historicalSectorsSpeakerBtn');
    if (!historicalSectorsSpeakerBtn) {
        console.error('Historical sectors speaker button not found');
        showFeedback('Historical sectors speaker button not found.', 'error');
        return;
    }
    ...
    if (window.speechSynthesis.speaking) {
        ...
        // Update button state
        const icon = historicalSectorsSpeakerBtn.querySelector('i');
        if (icon) icon.className = 'fas fa-volume-up';
        historicalSectorsSpeakerBtn.setAttribute('aria-label', 'Listen to Historical Change in Sectors story');

        return;
    }

    // Start historical sectors narration
    console.log('Starting historical sectors narration...');

    // Get the full content from the Historical Change in Sectors card
    const historicalSectorsCard = historicalSectorsSpeakerBtn.closest('.feature-card');
    if (!historicalSectorsCard) {
        console.error('Historical sectors card not found');
        showFeedback('Historical sectors card not found.', 'error');
        return;
    }

    // Get the feature-desc content
    const featureDesc = historicalSectorsCard.querySelector('.feature-desc');
    if (!featureDesc) {
        console.error('Feature description not found in Historical sectors card');
        showFeedback('Content not found in Historical sectors card.', 'error');
        return;
    }
    ...
    const textContent = featureDesc.textContent.trim();
    if (!textContent) {
        console.error('No text content found in Historical sectors card');
        showFeedback('No content found to narrate.', 'error');
        return;
    }

    console.log('Historical sectors text content to narrate:', textContent.substring(0, 100) + '...');

    // Create utterance for the content
    console.log('Creating utterance for main content...');
    const utterance = new SpeechSynthesisUtterance(textContent);
    utterance.rate = 0.9;
    utterance.pitch = 1.0;
    ...
    // Update button state to show it's playing
    historicalSectorsSpeakerBtn.classList.add('playing');
    const icon = historicalSectorsSpeakerBtn.querySelector('i');
    if (icon) icon.className = 'fas fa-stop';
    historicalSectorsSpeakerBtn.setAttribute('aria-label', 'Stop narration');

    // When narration starts
    utterance.onstart = () => {
        console.log('Historical sectors narration started successfully');
        currentSpeechState = 'speaking';
        updatePlayPauseButton();
    };
    ...
    // When narration ends
    utterance.onend = () => {
        console.log('Historical sectors narration ended successfully');
        currentSpeechState = 'stopped';
        updatePlayPauseButton();

        // Update button state
        historicalSectorsSpeakerBtn.classList.remove('playing');
        if (icon) icon.className = 'fas fa-volume-up';
        historicalSectorsSpeakerBtn.setAttribute('aria-label', 'Listen to Historical Change in Sectors story');
    };
    ...
    // Error handling
    utterance.onerror = (event) => {
        console.error('Historical sectors narration error:', event);
        currentSpeechState = 'stopped';
        updatePlayPauseButton();
    ...
    // Start speaking
    try {
        console.log('Attempting to start historical sectors narration...');
        window.speechSynthesis.speak(utterance);
        console.log('Historical sectors narration started successfully');
    } catch (error) {
        console.error('Error starting historical sectors narration:', error);
        currentSpeechState = 'stopped';
        updatePlayPauseButton();
...
// Toggle sectors narration function
function toggleSectorsNarration() {
   console.log('toggleSectorsNarration called');
   ...
   // Get the sectors speaker button
   const sectorsSpeakerBtn = document.getElementById('sectorsSpeakerBtn');
   if (!sectorsSpeakerBtn) {
       console.error('Sectors speaker button not found');
       showFeedback('Sectors speaker button not found.', 'error');
       return;
   }
   ...
   if (window.speechSynthesis.speaking) {
       ...
       // Update button state
       const icon = sectorsSpeakerBtn.querySelector('i');
       if (icon) icon.className = 'fas fa-volume-up';
       sectorsSpeakerBtn.setAttribute('aria-label', 'Listen to Rising Importance of Tertiary Sector story');

       return;
   }

   // Start sectors narration
   console.log('Starting sectors narration...');

   // Get the full content from the Sectors card
   const sectorsCard = sectorsSpeakerBtn.closest('.feature-card');
   if (!sectorsCard) {
       console.error('Sectors card not found');
       showFeedback('Sectors card not found.', 'error');
       return;
   }

   // Get the feature-desc content
   const featureDesc = sectorsCard.querySelector('.feature-desc');
   if (!featureDesc) {
       console.error('Feature description not found in Sectors card');
       showFeedback('Content not found in Sectors card.', 'error');
       return;
   }

   // Get all text content from the feature description, excluding speaker container
   const speakerContainer = featureDesc.querySelector('.sectors-speaker-container');
   let textContent = featureDesc.textContent.trim();

   // Remove speaker container text from narration
   if (speakerContainer) {
       const speakerText = speakerContainer.textContent.trim();
       textContent = textContent.replace(speakerText, '').trim();
   }

   if (!textContent) {
       console.error('No text content found in Sectors card');
       showFeedback('No content found to narrate.', 'error');
       return;
   }

   console.log('Sectors text content to narrate:', textContent.substring(0, 100) + '...');
   ...
   // Update button state to show it's playing
   sectorsSpeakerBtn.classList.add('playing');
   const icon = sectorsSpeakerBtn.querySelector('i');
   if (icon) icon.className = 'fas fa-stop';
   sectorsSpeakerBtn.setAttribute('aria-label', 'Stop narration');

   // When narration starts
   utterance.onstart = () => {
       console.log('Sectors narration started successfully');
       currentSpeechState = 'speaking';
       updatePlayPauseButton();
   };
   ...
   // When narration ends
   utterance.onend = () => {
       console.log('Sectors narration ended successfully');
       currentSpeechState = 'stopped';
       updatePlayPauseButton();

       // Update button state
       sectorsSpeakerBtn.classList.remove('playing');
       if (icon) icon.className = 'fas fa-volume-up';
       sectorsSpeakerBtn.setAttribute('aria-label', 'Listen to Rising Importance of Tertiary Sector story');
   };
   ...
   // Error handling
   utterance.onerror = (event) => {
       console.error('Sectors narration error:', event);
       currentSpeechState = 'stopped';
       updatePlayPauseButton();
   ...
   // Start speaking
   try {
       console.log('Attempting to start sectors narration...');
       window.speechSynthesis.speak(utterance);
       console.log('Sectors narration started successfully');
   } catch (error) {
       console.error('Error starting sectors narration:', error);
       currentSpeechState = 'stopped';
       updatePlayPauseButton();
...
// Toggle employment narration function
function toggleEmploymentNarration() {
   console.log('toggleEmploymentNarration called');
   ...
   // Get the employment speaker button
   const employmentSpeakerBtn = document.getElementById('employmentSpeakerBtn');
   if (!employmentSpeakerBtn) {
       console.error('Employment speaker button not found');
       showFeedback('Employment speaker button not found.', 'error');
       return;
   }
   ...
   if (window.speechSynthesis.speaking) {
       ...
       // Update button state
       const icon = employmentSpeakerBtn.querySelector('i');
       if (icon) icon.className = 'fas fa-volume-up';
       employmentSpeakerBtn.setAttribute('aria-label', 'Listen to Where are most people employed story');

       return;
   }

   // Start employment narration
   console.log('Starting employment narration...');

   // Get the full content from the Employment card
   const employmentCard = employmentSpeakerBtn.closest('.feature-card');
   if (!employmentCard) {
       console.error('Employment card not found');
       showFeedback('Employment card not found.', 'error');
       return;
   }

   // Get the feature-desc content
   const featureDesc = employmentCard.querySelector('.feature-desc');
   if (!featureDesc) {
       console.error('Feature description not found in Employment card');
       showFeedback('Content not found in Employment card.', 'error');
       return;
   }

   // Get all text content from the feature description, excluding speaker container
   const speakerContainer = featureDesc.querySelector('.employment-speaker-container');
   let textContent = featureDesc.textContent.trim();
   ...
   if (!textContent) {
       console.error('No text content found in Employment card');
       showFeedback('No content found to narrate.', 'error');
       return;
   }

   console.log('Employment text content to narrate:', textContent.substring(0, 100) + '...');
   ...
   // Update button state to show it's playing
   employmentSpeakerBtn.classList.add('playing');
   const icon = employmentSpeakerBtn.querySelector('i');
   if (icon) icon.className = 'fas fa-stop';
   employmentSpeakerBtn.setAttribute('aria-label', 'Stop narration');

   // When narration starts
   utterance.onstart = () => {
       console.log('Employment narration started successfully');
       currentSpeechState = 'speaking';
       updatePlayPauseButton();
   };
   ...
   // When narration ends
   utterance.onend = () => {
       console.log('Employment narration ended successfully');
       currentSpeechState = 'stopped';
       updatePlayPauseButton();

       // Update button state
       employmentSpeakerBtn.classList.remove('playing');
       if (icon) icon.className = 'fas fa-volume-up';
       employmentSpeakerBtn.setAttribute('aria-label', 'Listen to Where are most people employed story');
   };
   ...
   // Error handling
   utterance.onerror = (event) => {
       console.error('Employment narration error:', event);
       currentSpeechState = 'stopped';
       updatePlayPauseButton();
   ...
   // Start speaking
   try {
       console.log('Attempting to start employment narration...');
       window.speechSynthesis.speak(utterance);
       console.log('Employment narration started successfully');
   } catch (error) {
       console.error('Error starting employment narration:', error);
       currentSpeechState = 'stopped';
       updatePlayPauseButton();
...
// Toggle organised sector narration function
function toggleOrganisedNarration() {
    console.log('toggleOrganisedNarration called');
    ...
    // Get the organised speaker button
    const organisedSpeakerBtn = document.getElementById('organisedSpeakerBtn');
    if (!organisedSpeakerBtn) {
        console.error('Organised speaker button not found');
        showFeedback('Organised speaker button not found.', 'error');
        return;
    }
    ...
    if (window.speechSynthesis.speaking) {
        ...
        // Update button state
        const icon = organisedSpeakerBtn.querySelector('i');
        if (icon) icon.className = 'fas fa-volume-up';
        organisedSpeakerBtn.setAttribute('aria-label', 'Listen to Organised and Unorganised Sectors story');

        return;
    }

    // Start organised narration
    console.log('Starting organised narration...');

    // Get the full content from the Organised Sector card
    const organisedCard = organisedSpeakerBtn.closest('.feature-card');
    if (!organisedCard) {
        console.error('Organised card not found');
        showFeedback('Organised card not found.', 'error');
        return;
    }

    // Get the feature-desc content
    const featureDesc = organisedCard.querySelector('.feature-desc');
    if (!featureDesc) {
        console.error('Feature description not found in Organised card');
        showFeedback('Content not found in Organised card.', 'error');
        return;
    }

    // Get all text content from the feature description, excluding speaker container
    const speakerContainer = featureDesc.querySelector('.organised-speaker-container');
    let textContent = featureDesc.textContent.trim();
    ...
    if (!textContent) {
        console.error('No text content found in Organised card');
        showFeedback('No content found to narrate.', 'error');
        return;
    }

    console.log('Organised text content to narrate:', textContent.substring(0, 100) + '...');
    ...
    // Update button state to show it's playing
    organisedSpeakerBtn.classList.add('playing');
    const icon = organisedSpeakerBtn.querySelector('i');
    if (icon) icon.className = 'fas fa-stop';
    organisedSpeakerBtn.setAttribute('aria-label', 'Stop narration');

    // When narration starts
    utterance.onstart = () => {
        console.log('Organised narration started successfully');
        currentSpeechState = 'speaking';
        updatePlayPauseButton();
    };
    ...
    // When narration ends
    utterance.onend = () => {
        console.log('Organised narration ended successfully');
        currentSpeechState = 'stopped';
        updatePlayPauseButton();

        // Update button state
        organisedSpeakerBtn.classList.remove('playing');
        if (icon) icon.className = 'fas fa-volume-up';
        organisedSpeakerBtn.setAttribute('aria-label', 'Listen to Organised and Unorganised Sectors story');
    };
    ...
    // Error handling
    utterance.onerror = (event) => {
        console.error('Organised narration error:', event);
        currentSpeechState = 'stopped';
        updatePlayPauseButton();
    ...
    // Start speaking
    try {
        console.log('Attempting to start organised narration...');
        window.speechSynthesis.speak(utterance);
        console.log('Organised narration started successfully');
    } catch (error) {
        console.error('Error starting organised narration:', error);
        currentSpeechState = 'stopped';
        updatePlayPauseButton();
...
// Toggle protection narration function
function toggleProtectionNarration() {
    console.log('toggleProtectionNarration called');
    ...
    // Get the protection speaker button
    const protectionSpeakerBtn = document.getElementById('protectionSpeakerBtn');
    if (!protectionSpeakerBtn) {
        console.error('Protection speaker button not found');
        showFeedback('Protection speaker button not found.', 'error');
        return;
    }
    ...
    if (window.speechSynthesis.speaking) {
        ...
        // Update button state
        const icon = protectionSpeakerBtn.querySelector('i');
        if (icon) icon.className = 'fas fa-volume-up';
        protectionSpeakerBtn.setAttribute('aria-label', 'Listen to Protection of Unorganised Workers story');

        return;
    }

    // Start protection narration
    console.log('Starting protection narration...');

    // Get the full content from the Protection card
    const protectionCard = protectionSpeakerBtn.closest('.feature-card');
    if (!protectionCard) {
        console.error('Protection card not found');
        showFeedback('Protection card not found.', 'error');
        return;
    }

    // Get the feature-desc content
    const featureDesc = protectionCard.querySelector('.feature-desc');
    if (!featureDesc) {
        console.error('Feature description not found in Protection card');
        showFeedback('Content not found in Protection card.', 'error');
        return;
    }

    // Get all text content from the feature description, excluding speaker container
    const speakerContainer = featureDesc.querySelector('.protection-speaker-container');
    let textContent = featureDesc.textContent.trim();
    ...
    if (!textContent) {
        console.error('No text content found in Protection card');
        showFeedback('No content found to narrate.', 'error');
        return;
    }

    console.log('Protection text content to narrate:', textContent.substring(0, 100) + '...');
    ...
    // Update button state to show it's playing
    protectionSpeakerBtn.classList.add('playing');
    const icon = protectionSpeakerBtn.querySelector('i');
    if (icon) icon.className = 'fas fa-stop';
    protectionSpeakerBtn.setAttribute('aria-label', 'Stop narration');

    // When narration starts
    utterance.onstart = () => {
        console.log('Protection narration started successfully');
        currentSpeechState = 'speaking';
        updatePlayPauseButton();
    };
    ...
    // When narration ends
    utterance.onend = () => {
        console.log('Protection narration ended successfully');
        currentSpeechState = 'stopped';
        updatePlayPauseButton();

        // Update button state
        protectionSpeakerBtn.classList.remove('playing');
        if (icon) icon.className = 'fas fa-volume-up';
        protectionSpeakerBtn.setAttribute('aria-label', 'Listen to Protection of Unorganised Workers story');
    };
    ...
    // Error handling
    utterance.onerror = (event) => {
        console.error('Protection narration error:', event);
        currentSpeechState = 'stopped';
        updatePlayPauseButton();
    ...
    // Start speaking
    try {
        console.log('Attempting to start protection narration...');
        window.speechSynthesis.speak(utterance);
        console.log('Protection narration started successfully');
    } catch (error) {
        console.error('Error starting protection narration:', error);
        currentSpeechState = 'stopped';
        updatePlayPauseButton();
...
// Toggle comparing narration function
function toggleComparingNarration() {
    console.log('toggleComparingNarration called');
    ...
    // Get the comparing speaker button
    const comparingSpeakerBtn = document.getElementById('comparingSpeakerBtn');
    if (!comparingSpeakerBtn) {
        console.error('Comparing speaker button not found');
        showFeedback('Comparing speaker button not found.', 'error');
        return;
    }
    ...
    if (window.speechSynthesis.speaking) {
        ...
        // Update button state
        const icon = comparingSpeakerBtn.querySelector('i');
        if (icon) icon.className = 'fas fa-volume-up';
        comparingSpeakerBtn.setAttribute('aria-label', 'Click to listen to the content');

        return;
    }

    // Start comparing narration
    console.log('Starting comparing narration...');

    // The specific text content to narrate
    const textContent = "The various production activities in the primary, secondary and tertiary sectors produce a very large number of goods and services. Also, the three sectors have a large number of people working in them to produce these goods and services. The next step, therefore, is to see how much goods and services are produced and how many people work in each sector. In an economy there could be one or more sectors which are dominant in terms of total production and employment, while other sectors are relatively small in size.";

    if (!textContent) {
        console.error('No text content found to narrate');
        showFeedback('No content found to narrate.', 'error');
        return;
    }

    console.log('Comparing text content to narrate:', textContent.substring(0, 100) + '...');
    ...
    // Update button state to show it's playing
    comparingSpeakerBtn.classList.add('playing');
    const icon = comparingSpeakerBtn.querySelector('i');
    if (icon) icon.className = 'fas fa-stop';
    comparingSpeakerBtn.setAttribute('aria-label', 'Stop narration');

    // When narration starts
    utterance.onstart = () => {
        console.log('Comparing narration started successfully');
        currentSpeechState = 'speaking';
        updatePlayPauseButton();
    };
    ...
    // When narration ends
    utterance.onend = () => {
        console.log('Comparing narration ended successfully');
        currentSpeechState = 'stopped';
        updatePlayPauseButton();

        // Update button state
        comparingSpeakerBtn.classList.remove('playing');
        if (icon) icon.className = 'fas fa-volume-up';
        comparingSpeakerBtn.setAttribute('aria-label', 'Click to listen to the content');
    };
    ...
    // Error handling
    utterance.onerror = (event) => {
        console.error('Comparing narration error:', event);
        currentSpeechState = 'stopped';
        updatePlayPauseButton();
    ...
    // Start speaking
    try {
        console.log('Attempting to start comparing narration...');
        window.speechSynthesis.speak(utterance);
        console.log('Comparing narration started successfully');
    } catch (error) {
        console.error('Error starting comparing narration:', error);
        currentSpeechState = 'stopped';
        updatePlayPauseButton();
...
// Toggle create employment narration function
function toggleCreateEmploymentNarration() {
    console.log('=== TOGGLE CREATE EMPLOYMENT NARRATION CALLED ===');
    console.log('toggleCreateEmploymentNarration called');
    console.log('isAudioEnabled:', isAudioEnabled);
    console.log('window.speechSynthesis available:', !!window.speechSynthesis);

    // Show immediate feedback that function was called
    showFeedback('Create Employment narration function called!', 'info');

    // Check if audio is globally enabled
    console.log('Checking if audio is enabled...');
    console.log('isAudioEnabled value:', isAudioEnabled);
    console.log('globalNarrationState.isEnabled:', globalNarrationState.isEnabled);
   ...
   // Get the create employment speaker button
   const createEmploymentSpeakerBtn = document.getElementById('createEmploymentSpeakerBtn');
   console.log('Button element found:', !!createEmploymentSpeakerBtn);
   if (!createEmploymentSpeakerBtn) {
       console.error('Create employment speaker button not found');
       showFeedback('Create employment speaker button not found.', 'error');
       return;
   }

   // Add a simple click test
   console.log('Button click event test - button exists and should be clickable');

   // Check if currently speaking
   console.log('Checking if currently speaking...');
   console.log('window.speechSynthesis.speaking:', window.speechSynthesis.speaking);
   console.log('window.speechSynthesis.pending:', window.speechSynthesis.pending);
   ...
   if (window.speechSynthesis.speaking || window.speechSynthesis.pending) {
       ...
       // Update button state
       const icon = createEmploymentSpeakerBtn.querySelector('i');
       if (icon) icon.className = 'fas fa-volume-up';
       createEmploymentSpeakerBtn.setAttribute('aria-label', 'Listen to How to Create More Employment story');

       console.log('Narration stopped successfully');
       return;
   }

   // Start create employment narration
   console.log('Starting create employment narration...');

   // Get the full content from the Create Employment card
   const createEmploymentCard = createEmploymentSpeakerBtn.closest('.feature-card');
   console.log('Create employment card found:', !!createEmploymentCard);
   if (!createEmploymentCard) {
       console.error('Create employment card not found');
       showFeedback('Create employment card not found.', 'error');
       return;
   }

   // Get the feature-desc content
   const featureDesc = createEmploymentCard.querySelector('.feature-desc');
   console.log('Feature description found:', !!featureDesc);
   if (!featureDesc) {
       console.error('Feature description not found in Create Employment card');
       showFeedback('Content not found in Create Employment card.', 'error');
       return;
   }

   // Get all text content from the feature description, excluding speaker container
   const speakerContainer = featureDesc.querySelector('.create-employment-speaker-container');
   let textContent = featureDesc.textContent.trim();
   ...
   if (!textContent) {
       console.error('No text content found in Create Employment card');
       showFeedback('No content found to narrate.', 'error');
       return;
   }

   console.log('Create employment text content to narrate:', textContent.substring(0, 100) + '...');
   ...
   // Update button state to show it's playing
   createEmploymentSpeakerBtn.classList.add('playing');
   const icon = createEmploymentSpeakerBtn.querySelector('i');
   if (icon) icon.className = 'fas fa-stop';
   createEmploymentSpeakerBtn.setAttribute('aria-label', 'Stop narration');

   // When narration starts
   utterance.onstart = () => {
       console.log('Create employment narration started successfully');
       currentSpeechState = 'speaking';
       updatePlayPauseButton();
   };
   ...
   // When narration ends
   utterance.onend = () => {
       console.log('Create employment narration ended successfully');
       currentSpeechState = 'stopped';
       updatePlayPauseButton();

       // Update button state
       createEmploymentSpeakerBtn.classList.remove('playing');
       if (icon) icon.className = 'fas fa-volume-up';
       createEmploymentSpeakerBtn.setAttribute('aria-label', 'Listen to How to Create More Employment story');
   };
   ...
   // Error handling
   utterance.onerror = (event) => {
       console.error('Create employment narration error:', event);
       currentSpeechState = 'stopped';
       updatePlayPauseButton();
   ...
   setTimeout(() => {
       try {
           // Double-check that speech synthesis is clean
           if (window.speechSynthesis && !window.speechSynthesis.speaking && !window.speechSynthesis.pending) {
               window.speechSynthesis.speak(utterance);
               console.log('Create employment narration started successfully');
           } else {
               console.log('Speech synthesis still busy, retrying...');
               // Try one more time after a longer delay
               setTimeout(() => {
                   window.speechSynthesis.speak(utterance);
                   console.log('Create employment narration started on retry');
               }, 200);
           }
       } catch (error) {
           console.error('Error starting narration:', error);
           showFeedback('Error starting narration. Please try again.', 'error');
