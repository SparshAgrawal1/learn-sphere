/**
 * Activities for The Lost Child interactive lesson
 */
...
function checkVocabulary() {
    const correctAnswers = {
        'vocab1': 'a', // fascinated - Extremely interested and attracted by something
        'vocab2': 'b', // tyrant - A cruel, unfair, or strict ruler
        'vocab3': 'b', // jostled - Pushed roughly or bumped against in a crowd
        'vocab4': 'c', // disconsolate - Very unhappy and unable to be comforted
        'vocab5': 'a'  // throngs - Large crowds of people
    };
...
function checkDescriptives() {
    // These are sample answers - multiple correct answers are possible
    const possibleAnswers = {
        0: ['gaily colored', 'colorful', 'bright'],
        1: ['red eyes', 'red-eyed', 'familiar tyrant\'s way', 'red eyes in his familiar tyrant\'s way'],
        2: ['melting gold', 'pale like melting gold', 'gold'],
        3: ['gaudy purple wings', 'purple wings', 'gaudy wings'],
        4: ['yellow turban', 'turban', 'clothes']
    };
    ...
    inputs.forEach((input, index) => {
        ...
        // Only check for matches if the input is not empty
        if (userAnswer !== '') {
            isCorrect = possibleAnswers[index].some(answer => 
    ...
    // Display feedback
    const feedbackEl = document.getElementById('descriptivesFeedback');
    if (feedbackEl) {
        if (score === total) {
            feedbackEl.textContent = `Perfect! You completed all ${total} descriptive phrases correctly.`;
            feedbackEl.className = 'feedback-message show success';
            ...
            if (typeof modulesCompleted !== 'undefined' && !modulesCompleted.includes('thinking-language')) {
                ...
                if (typeof showAchievement === 'function') {
                    showAchievement('Descriptives Exercise Completed!');
                }
            }
        } else {
            feedbackEl.textContent = `You got ${score} out of ${total} correct. Try again!`;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const descriptivesButton = document.querySelector('button[onclick="checkDescriptives()"]');
    if (descriptivesButton) {
        descriptivesButton.addEventListener('click', checkDescriptives);
    }
});
