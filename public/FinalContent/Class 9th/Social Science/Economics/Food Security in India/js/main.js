...
let moduleProgress = {
     ...
     foodinsecure: false,
     foodsecurityindia: false,
     bufferstock: false,
     pdsystem: false,
     currentstatuspds: false,
...
const moduleOrder = ['intro', 'overview', 'southafrica', 'whyconstitution', 'foodinsecure', 'foodsecurityindia', 'bufferstock', 'pdsystem', 'currentstatuspds', 'indianconstitution', 'summingup', 'activities'];
...
function startModuleNarration(moduleId) {
    ...
    switch(moduleId) {
        case 'intro':
            speak("Welcome to Food Security in India. This chapter explores food security as the assurance that all people have access to sufficient, safe, and nutritious food at all times. It examines the three key dimensions: food availability, affordability, and proper distribution systems.");
            break;
            
        case 'overview':
            speak("This chapter explores food security in India, focusing on the availability, accessibility, and affordability of food for all people at all times.");
            break;
            
        case 'southafrica':
            speak("This section explains the meaning and dimensions of food security in India. Food security goes beyond just having enough to eat - it ensures food availability, accessibility, and affordability for all people at all times. The section covers the three key dimensions: availability of food through production, imports and government stocks; accessibility meaning food is within reach of everyone; and affordability ensuring people have enough money to buy sufficient, safe and nutritious food.");
            break;
            
        case 'whyconstitution':
            speak("This section explains why food security is essential for India, particularly how natural calamities and pandemics can create food insecurity even for people above the poverty line. It covers the devastating impact of famines like the Bengal Famine of 1943, the evolution of food security concepts from the 1970s to the present, including Amartya Sen's entitlements approach and the comprehensive 1995 World Food Summit definition that emphasizes physical and economic access to sufficient, safe and nutritious food.");
            break;

        case 'foodinsecure':
            speak("This section identifies who are most vulnerable to food insecurity in India, including landless people, traditional artisans, petty self-employed workers, and socially disadvantaged groups. It explores hunger dimensions - both chronic and seasonal - and discusses India's journey towards self-sufficiency in foodgrains through the Green Revolution, with stories of Ramu, a seasonal agricultural laborer, and Ahmad, a rickshaw puller in Bangalore.");
            break;

        case 'foodsecurityindia':
            speak("This section explains how India's food security system has evolved since the Green Revolution in the early 1970s. It covers how the country has achieved self-sufficiency in foodgrains and avoided famines even during adverse weather conditions through a well-designed food security system with two key components: buffer stock and the public distribution system.");
            break;

        case 'bufferstock':
            speak("This section explains the concept of buffer stock in India's food security system. Buffer stock consists of foodgrains like wheat and rice procured by the Food Corporation of India from surplus states at Minimum Support Price. This stock is stored in granaries and distributed to deficit areas and poorer sections at subsidized Issue Price, helping to resolve food shortages during adverse weather conditions or calamities.");
            break;

        case 'pdsystem':
            speak("This section explains the Public Distribution System (PDS) in India, which distributes food procured by FCI through government-regulated ration shops to poorer sections of society. It covers the three types of ration cards (Antyodaya, BPL, APL), the historical evolution of the PDS since the 1940s Bengal famine, and the National Food Security Act 2013 that provides food and nutritional security to 75% of rural and 50% of urban populations.");
            break;

        case 'currentstatuspds':
            speak("This section examines the current status of India's Public Distribution System, including its evolution from universal coverage to targeted systems like RPDS and TPDS. It discusses the system's effectiveness in stabilizing prices and preventing hunger, while also highlighting challenges such as overflowing FCI godowns, high buffer stocks, environmental concerns from intensive rice and wheat cultivation, and issues with PDS dealers. The section also covers the impact of minimum support prices on production patterns and the decline in per capita consumption of foodgrains.");
            break;

        case 'indianconstitution':
            speak("Cooperatives play a crucial role in ensuring food security in India, particularly in southern and western regions. Through cooperative societies, fair price shops provide low-cost goods to poor people, with 94% of Tamil Nadu's fair price shops run by cooperatives. Success stories include Mother Dairy in Delhi and Amul in Gujarat, which brought about the White Revolution. Additionally, organizations like the Academy of Development Science in Maharashtra have established grain banks and conducted training programs, creating innovative food security interventions that benefit various sections of society.");
            break;
            

        case 'summingup':
            speak("Food security in India is ensured when all citizens have access to nutritious food, the capacity to purchase quality food, and no barriers to food access. While people below the poverty line face constant food insecurity, even better-off individuals can become food insecure during calamities. The government has implemented a comprehensive food security system including buffer stocks and the Public Distribution System, along with various programs like ICDS, Food-for-Work, Mid-Day Meals, and Antyodaya Anna Yojana. Additionally, cooperatives and NGOs play a crucial role in enhancing food security across different regions of the country.");
            break;

        case 'activities':
            speak("Test your knowledge and understanding with these interactive activities about Food Security in India.");
            break;
...
function startJourney() {
    ...
    showAchievement('Journey Begins', 'You started your learning journey with Poverty as a Challenge!');
    speak("Welcome to Food Security in India. This chapter explores food security as the assurance that all people have access to sufficient, safe, and nutritious food at all times. It examines the three key dimensions: food availability, affordability, and proper distribution systems. The lesson covers why food security is crucial for national well-being, with special focus on vulnerable populations and the critical role of government initiatives like the Public Distribution System in preventing hunger and maintaining social stability.");
}
...
function checkQuizAnswers() {
    ...
    if (q1Answer && q1Answer.value === 'b') {
        score++;
        results.push('<p class="correct-answer">✓ Question 1: Correct! The past two decades of globalisation has seen rapid movements in goods, services and investments between countries.</p>');
    } else {
        results.push('<p class="wrong-answer">✗ Question 1: Incorrect. The correct answer is b) goods, services and investments between countries.</p>');
    }
    ...
    if (q2Answer && q2Answer.value === 'b') {
        score++;
        results.push('<p class="correct-answer">✓ Question 2: Correct! The most common route for investments by MNCs is to buy existing local companies.</p>');
    } else {
        results.push('<p class="wrong-answer">✗ Question 2: Incorrect. The correct answer is b) buy existing local companies.</p>');
    }
    ...
    if (q3Answer && q3Answer.value === 'd') {
        score++;
        results.push('<p class="correct-answer">✓ Question 3: Correct! Globalisation has not led to improvement in living conditions of all the people.</p>');
    } else {
        results.push('<p class="wrong-answer">✗ Question 3: Incorrect. The correct answer is d) none of the above.</p>');
    }
...
function toggleProductionNarration() {
    ...
    const curatedContent = contentElements.length > 0
        ...
        : "The aim of production is to produce the goods and services that we want. There are four requirements for production of goods and services. The first requirement is land, and other natural resources such as water, forests, minerals. The second requirement is labour, i.e. people who will do the work. Some production activities require highly educated workers to perform the necessary tasks. Other activities require workers who can do manual work. Each worker is providing the labour necessary for production. The third requirement is physical capital, i.e. the variety of inputs required at every stage during production. Tools, machines, buildings can be used in production over many years, and are called fixed capital. Raw materials and money in hand are called working capital. Unlike tools, machines and buildings, these are used up in production. There is a fourth requirement too. You will need knowledge and enterprise to be able to put together land, labour and physical capital and produce an output either to use yourself or to sell in the market. This these days is called human capital. Every production is organised by combining land, labour, physical capital and human capital, which are known as factors of production.";
...
// ====== Poverty Cards Narration Functions ======

// Card 1: Urban Case
function toggleUrbanCaseNarration() {
    const btn = document.getElementById('urbanCaseSpeakerBtn');
    if (!btn) return;
...
// Card 2: Rural Case
function toggleRuralCaseNarration() {
    const btn = document.getElementById('ruralCaseSpeakerBtn');
    if (!btn) return;
...
// Card 3: Poverty as seen by social scientists
function toggleSocialScientistsNarration() {
    const btn = document.getElementById('socialScientistsSpeakerBtn');
    if (!btn) return;
...
// Card 4: Social Exclusion
function toggleSocialExclusionNarration() {
    const btn = document.getElementById('socialExclusionSpeakerBtn');
    if (!btn) return;
...
// Card 5: Vulnerability
function toggleVulnerabilityNarration() {
    const btn = document.getElementById('vulnerabilitySpeakerBtn');
    if (!btn) return;
...
// Card 6: Poverty Line
function togglePovertyLineNarration() {
    const btn = document.getElementById('povertyLineSpeakerBtn');
    if (!btn) return;
...
// Card 7: Poverty Estimates
function togglePovertyEstimatesNarration() {
    const btn = document.getElementById('povertyEstimatesSpeakerBtn');
    if (!btn) return;
...
// Card 8: Inter-State Disparities
function toggleInterStateNarration() {
    const btn = document.getElementById('interStateSpeakerBtn');
    if (!btn) return;
...
// Card 9: Vulnerable Groups
function toggleVulnerableGroupsNarration() {
    const btn = document.getElementById('vulnerableGroupsSpeakerBtn');
    if (!btn) return;
...
// Card 10: Global Poverty Scenario
function toggleGlobalPovertyNarration() {
    const btn = document.getElementById('globalPovertySpeakerBtn');
    if (!btn) return;
...
// Card 11: Poverty and Sustainable Development Goals
function toggleSDGNarration() {
    const btn = document.getElementById('sdgSpeakerBtn');
    if (!btn) return;
...
// Card 12: Causes of Poverty
function toggleCausesNarration() {
    const btn = document.getElementById('causesSpeakerBtn');
    if (!btn) return;
...
// Card 13: Anti-Poverty Measures
function toggleAntiPovertyNarration() {
    const btn = document.getElementById('antiPovertySpeakerBtn');
    if (!btn) return;
...
function toggleIntroNarration() {
    ...
    // Use the new content for Food Security in India
    const curatedContent = "Food security means ensuring that everyone has access to sufficient, safe, and nutritious food at all times. It depends on the availability of food, people's ability to afford it, and an effective system for distribution. This chapter explains how food security is vital for a nation's well-being, especially for poor households that are most vulnerable during crises. It also highlights the role of the Public Distribution System (PDS) and government efforts in maintaining stability and preventing hunger.";
...
// ====== Non-Farm Activities in Palampur Card Narration Functions ======

// Card 1: Dairy
function toggleDairyNarration() {
    const btn = document.getElementById('dairySpeakerBtn');
    if (!btn) return;
...
// Card 2: Small-scale manufacturing
function toggleManufacturingNarration() {
    const btn = document.getElementById('manufacturingSpeakerBtn');
    if (!btn) return;
...
// Card 3: Shopkeepers
function toggleShopkeepersNarration() {
    const btn = document.getElementById('shopkeepersSpeakerBtn');
    if (!btn) return;
...
// Card 4: Transport
function toggleTransportNarration() {
    const btn = document.getElementById('transportSpeakerBtn');
    if (!btn) return;
...
// ====== Farming in Palampur Card Narration Functions ======

// Card 1: Land is Fixed
function toggleLandNarration() {
    const btn = document.getElementById('landSpeakerBtn');
    if (!btn) return;
...
// Card 2: Multiple Cropping and Modern Farming
function toggleCropsNarration() {
    const btn = document.getElementById('cropsSpeakerBtn');
    if (!btn) return;
...
// Card 4: Land Distribution
function toggleDistributionNarration() {
    const btn = document.getElementById('distributionSpeakerBtn');
    if (!btn) return;
...
// Card 5: Labour in Farming
function toggleLabourNarration() {
    const btn = document.getElementById('labourSpeakerBtn');
    if (!btn) return;
...
// Card 6: Capital in Farming
function toggleCapitalNarration() {
    const btn = document.getElementById('capitalSpeakerBtn');
    if (!btn) return;
...
// Card 7: Sale of Surplus
function toggleSurplusNarration() {
    const btn = document.getElementById('surplusSpeakerBtn');
    if (!btn) return;
...
function toggleSouthAfricaNarration() {
    ...
    // Use the food security dimensions content
    const curatedContent = "Food is as essential for living as air is for breathing. But food security means something more than getting two square meals. Food security has following dimensions (a) availability of food means food production within the country, food imports and the previous years stock stored in government granaries. (b) accessibility means food is within reach of every person. (c) affordability implies that an individual has enough money to buy sufficient, safe and nutritious food to meet one's dietary needs. Thus, food security is ensured in a country only if (1) enough food is available for all the persons (2) all persons have the capacity to buy food of acceptable quality and (3) there is no barrier on access to food.";
...
// Who are food-insecure? section
function toggleFoodInsecureNarration() {
    const btn = document.getElementById('foodInsecureSpeakerBtn');
    if (!btn) return;
...
// Card 1: Story of Ramu
function toggleRamuNarration() {
    const btn = document.getElementById('ramuSpeakerBtn');
    if (!btn) return;
...
// Card 2: Story of Ahmad
function toggleAhmadNarration() {
    const btn = document.getElementById('ahmadSpeakerBtn');
    if (!btn) return;
...
// Food Security in India section
function toggleFoodSecurityIndiaNarration() {
    const btn = document.getElementById('foodSecurityIndiaSpeakerBtn');
    if (!btn) return;
...
// What is Buffer stock? section
function toggleBufferStockNarration() {
    const btn = document.getElementById('bufferStockSpeakerBtn');
    if (!btn) return;
...
// What is the Public Distribution System? section
function togglePDSystemNarration() {
    const btn = document.getElementById('pdSystemSpeakerBtn');
    if (!btn) return;
...
// Current Status of Public Distribution System section
function toggleCurrentStatusPDSNarration() {
    const btn = document.getElementById('currentStatusPDSSpeakerBtn');
    if (!btn) return;
    ...
    const contentElement = document.getElementById('currentstatuspds-content');
    if (!contentElement) {
        showFeedback('Content not found', 'error');
        return;
    }

    const textToNarrate = contentElement.textContent || contentElement.innerText || '';
    if (!textToNarrate.trim()) {
        showFeedback('No content to narrate', 'error');
        return;
    }
    ...
    const chunks = splitTextIntoChunks(textToNarrate, 2000);
    speakChunksSequentially(chunks, btn);
}
...
function speakChunksSequentially(chunks, btn) {
    let currentIndex = 0;
...
// Role of cooperatives in food security Narration
function toggleCooperativesNarration() {
    const btn = document.getElementById('cooperativesSpeakerBtn');
    if (!btn) return;
    ...
    try {
        ...
        console.error('Error speaking cooperatives content:', err);
        onCleanup();
    }
}
