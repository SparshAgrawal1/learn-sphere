...
function checkAnswer(optionIndex) {
    ...
    const correctAnswers = [1, 3, 2, 1]; // b, d, c, b (0-indexed)
...
function nextQuestion() {
    const totalQuestions = 4; // We have 4 questions in the HTML
...
function showCurrentQuestion() {
    ...
    const progressFill = document.getElementById('quiz-progress-fill');
    if (progressFill) {
        const progress = ((quizQuestionIndex + 1) / 4) * 100;
        progressFill.style.width = `${progress}%`;
        progressFill.textContent = `Question ${quizQuestionIndex + 1} of 4`;
    }
...
function showQuizResults() {
    ...
    // Question 1: b) Unequal roles assigned by the society to men and women (index 1)
    const q1Answer = document.querySelector('input[name="q1"]:checked');
    if (q1Answer && q1Answer.value === 'b') {
        correctCount++;
    }

    // Question 2: d) Panchayati Raj bodies (index 3)
    const q2Answer = document.querySelector('input[name="q2"]:checked');
    if (q2Answer && q2Answer.value === 'd') {
        correctCount++;
    }

    // Question 3: c) A and C (index 2)
    const q3Answer = document.querySelector('input[name="q3"]:checked');
    if (q3Answer && q3Answer.value === 'c') {
        correctCount++;
    }

    // Question 4: b) gives official status to one religion (index 1)
...
function checkMatchingAnswers() {
    ...
    // Define correct answers for gender, religion and caste matching
    const correctAnswers = {
        '1': 'b',  // A person who believes in equal rights and opportunities for women and men → Feminist
        '2': 'a',  // A person who says that religion is the principal basis of community → Communalist
        '3': 'd',  // A person who thinks that caste is the principal basis of community → Castiest
        '4': 'c'   // A person who does not discriminate others on the basis of religious beliefs → Secularist
    };
