...
function checkAnswer(optionIndex) {
    ...
    const correctAnswers = [1, 1, 3]; // b, b, d (0-indexed)
...
function showCurrentQuestion() {
    ...
    const progressFill = document.getElementById('quiz-progress-fill');
    if (progressFill) {
        const progress = ((quizQuestionIndex + 1) / 3) * 100;
        progressFill.style.width = `${progress}%`;
        progressFill.textContent = `Question ${quizQuestionIndex + 1} of 3`;
    }
...
function showQuizResults() {
    ...
    // Question 1: b) goods, services and investments between countries (index 1)
    const q1Answer = document.querySelector('input[name="q1"]:checked');
    if (q1Answer && q1Answer.value === 'b') {
        correctCount++;
    }

    // Question 2: b) buy existing local companies (index 1)
    const q2Answer = document.querySelector('input[name="q2"]:checked');
    if (q2Answer && q2Answer.value === 'b') {
        correctCount++;
    }

    // Question 3: d) none of the above (index 3)
...
function checkMatchingAnswers() {
    ...
    // Define correct answers for globalization matching
    const correctAnswers = {
        '1': 'b',  // MNCs buy at cheap rates from small producers → Garments, footwear, sports items
        '2': 'e',  // Quotas and taxes on imports are used to regulate trade → Trade barriers
        '3': 'd',  // Indian companies who have invested abroad → Tata Motors, Infosys, Ranbaxy
        '4': 'c',  // IT has helped in spreading of production of services → Call centres
        '5': 'a'   // Several MNCs have invested in setting up factories in India for production → Automobiles
    };
...
function submitAnswer(questionNumber) {
    ...
    // Special handling for Question 1 (Census Survey Form)
    if (questionNumber === 1) {
        const formInputs = [
            document.getElementById('location'),
            document.getElementById('total-area'),
            document.getElementById('irrigated-land'),
            document.getElementById('unirrigated-land'),
            ...
            document.getElementById('nearest-town')
        ];
        ...
        // Disable all form inputs and hide submit button, show edit button
...
function editAnswer(questionNumber) {
    ...
    if (questionNumber === 1) {
        ...
        // Enable all form inputs and show submit button, hide edit button
        formInputs.forEach(input => {
...
function checkAllQuestionsAnswered() {
    ...
    if (allAnswered) {
        ...
        showAchievement('Critical Thinker', 'You have completed all 14 critical thinking questions!');
