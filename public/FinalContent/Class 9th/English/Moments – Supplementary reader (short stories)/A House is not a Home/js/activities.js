/**
 * Activities for A House Is Not a Home interactive lesson
 */
...
function checkVocabulary() {
    const correctAnswers = {
        'vocab1': 'feeding and tending the fire',
        'vocab2': 'strange or bizarre',
        'vocab3': 'a dull and apathetic person',
        'vocab4': 'moving in an aimless manner',
        'vocab5': 'push hard'
    };
...
// Check prefix/suffix exercises
function checkConditionals() {
    // These are sample answers - multiple correct answers are possible for some
    ...
    if (feedbackEl) {
        if (score === total) {
            feedbackEl.textContent = `Perfect! You completed all ${total} prefix/suffix exercises correctly.`;
            feedbackEl.className = 'feedback-message show success';
            ...
            if (typeof modulesCompleted !== 'undefined' && !modulesCompleted.includes('thinking-language')) {
                ...
                if (typeof showAchievement === 'function') {
                    showAchievement('Prefixes/Suffixes Exercise Completed!');
                }
            }
        } else {
            feedbackEl.textContent = `You got ${score} out of ${total} correct. Try again!`;
