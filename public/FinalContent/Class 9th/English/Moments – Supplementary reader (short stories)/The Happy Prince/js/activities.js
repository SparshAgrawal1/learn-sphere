/**
 * Activities for The Happy Prince interactive lesson
 */
...
function checkVocabulary() {
    const correctAnswers = {
        'vocab1': 'covered in gold',
        'vocab2': 'landed',
        'vocab3': 'a woman who sews',
        'vocab4': 'small dark room at the top of the house',
        'vocab5': 'without energy or interest'
    };
...
// Check literary devices answers
function checkConditionals() {
    // These are sample answers - multiple correct answers are possible for literary devices
    const possibleAnswers = {
        0: ['imagery', 'description', 'visual imagery', 'descriptive language'],
        1: ['repetition', 'repeat', 'repeated phrase'],
        2: ['irony', 'situational irony', 'dramatic irony'],
        3: ['metaphor', 'personification', 'comparison'],
        4: ['symbolism', 'symbol', 'metaphor']
    };
    ...
    if (feedbackEl) {
        ...
        if (score === total) {
            feedbackEl.textContent = `Perfect! You identified all ${total} literary devices correctly.`;
            feedbackEl.className = 'feedback-message show success';
            ...
            if (typeof modulesCompleted !== 'undefined' && !modulesCompleted.includes('thinking-language')) {
                ...
                if (typeof showAchievement === 'function') {
                    showAchievement('Literary Devices Exercise Completed!');
                }
            }
        } else {
...
// Show correct answers for literary devices
function showCorrectLiteraryDevices() {
    const inputs = document.querySelectorAll('.contraction-input');
    const possibleAnswers = {
        0: ['imagery', 'description', 'visual imagery', 'descriptive language'],
        1: ['repetition', 'repeat', 'repeated phrase'],
    ...
    inputs.forEach((input, index) => {
        ...
        tooltip.textContent = `Answer: ${possibleAnswers[index][0]}`;
