...
const listeningAnswers = {
    'value': 'न्याय, स्वतंत्रता, समता और बंधुता',
    'narrative': '3'
};
...
function clearListeningFeedback() {
    ...
    // Remove answer icons (both standard and immediate feedback icons)
    const answerIcons = document.querySelectorAll('.option-item .answer-icon, .narrative-option .answer-icon, .option-item .immediate-feedback, .narrative-option .immediate-feedback');
    answerIcons.forEach(icon => icon.remove());
    
    // Remove answer styling (both standard and immediate feedback styling)
    const optionItems = document.querySelectorAll('.option-item, .narrative-option');
    optionItems.forEach(option => {
        option.classList.remove('correct-answer', 'incorrect-answer', 'correct-answer-immediate', 'incorrect-answer-immediate');
    });
}
...
// Add immediate feedback for narrative options
function setupImmediateFeedback() {
    // Get all narrative option radio buttons
    const narrativeRadios = document.querySelectorAll('input[name="narrative"]');
    const correctAnswer = listeningAnswers.narrative;
    
    // Add event listeners to each radio button
    narrativeRadios.forEach(radio => {
        radio.addEventListener('change', function() {
            // Remove any existing feedback icons
            document.querySelectorAll('.narrative-option .immediate-feedback').forEach(icon => icon.remove());
            
            // Remove existing feedback classes
            document.querySelectorAll('.narrative-option').forEach(option => {
                option.classList.remove('correct-answer-immediate', 'incorrect-answer-immediate');
            });
            
            // Get the selected option container
            const selectedOption = this.closest('.narrative-option');
            
            // Create feedback icon element
            const feedbackIcon = document.createElement('span');
            feedbackIcon.className = 'immediate-feedback';
            
            // Check if answer is correct
            if (this.value === correctAnswer) {
                // Correct answer
                selectedOption.classList.add('correct-answer-immediate');
                feedbackIcon.innerHTML = '✓';
                feedbackIcon.style.color = '#4CAF50'; // Green color
            } else {
                // Incorrect answer
                selectedOption.classList.add('incorrect-answer-immediate');
                feedbackIcon.innerHTML = '✗';
                feedbackIcon.style.color = '#F44336'; // Red color
                
                // Show the correct answer
                const correctOption = document.querySelector(`#narrative${correctAnswer}`).closest('.narrative-option');
                correctOption.classList.add('correct-answer-immediate');
                
                // Add check mark to correct option
                const correctFeedbackIcon = document.createElement('span');
                correctFeedbackIcon.className = 'immediate-feedback';
                correctFeedbackIcon.innerHTML = '✓';
                correctFeedbackIcon.style.color = '#4CAF50'; // Green color
                correctOption.appendChild(correctFeedbackIcon);
            }
            
            // Add the feedback icon to the selected option
            selectedOption.appendChild(feedbackIcon);
        });
    });
    
    // Similar setup for value options
    const valueRadios = document.querySelectorAll('input[name="value"]');
    const correctValueAnswer = listeningAnswers.value;
    
    valueRadios.forEach(radio => {
        radio.addEventListener('change', function() {
            // Remove any existing feedback icons
            document.querySelectorAll('.option-item .immediate-feedback').forEach(icon => icon.remove());
            ...
            // Check if answer is correct
            if (this.value === correctValueAnswer) {
                // Correct answer
                selectedOption.classList.add('correct-answer-immediate');
                feedbackIcon.innerHTML = '✓';
                feedbackIcon.style.color = '#4CAF50'; // Green color
                ...
                // Show the correct answer
                const correctOptionId = Array.from(valueRadios).find(radio => radio.value === correctValueAnswer).id;
                const correctOption = document.getElementById(correctOptionId).closest('.option-item');
                correctOption.classList.add('correct-answer-immediate');
...
document.addEventListener('DOMContentLoaded', function() {
    // Setup immediate feedback for options
