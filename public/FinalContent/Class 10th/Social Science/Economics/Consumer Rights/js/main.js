// Main JavaScript for Consumer Rights Interactive Lesson
...
let moduleProgress = {
    ...
    consumerorganisations: false,
    consumerforums: false,
    consumerresponsibilities: false,
    activities: false
};

// Module order for progress calculation
const moduleOrder = ['intro', 'overview', 'consumerprotection', 'consumerorganisations', 'consumerforums', 'consumerresponsibilities', 'activities'];
...
function startModuleNarration(moduleId) {
    ...
    switch(moduleId) {
        case 'intro':
            speak("Welcome to Consumer Rights. This chapter explores the challenges consumers face, such as unfair practices and weak enforcement of rules, and highlights the importance of being informed and active participants in the consumer movement.");
            break;
            
        case 'overview':
            speak("This chapter discusses consumer rights within the context of market operations in our country. It covers unequal situations in markets, poor enforcement of rules and regulations, and the need to encourage participation in the consumer movement through case histories and real-life examples.");
            break;

        case 'consumerprotection':
            speak("This section explores how we participate in markets as both producers and consumers, and examines the need for rules and regulations to protect consumers from exploitation and unfair trade practices in the marketplace.");
            break;

        case 'consumerorganisations':
            speak("The consumer movement arose out of dissatisfaction of consumers as many unfair practices were being indulged in by sellers. There was no legal system available to consumers to protect them from exploitation in the marketplace. In India, the consumer movement originated with the necessity of protecting and promoting the interests of consumers against unethical and unfair trade practices.");
            break;

        case 'consumerforums':
            speak("Consumer Rights are essential protections that help ensure fair treatment in the marketplace. This section covers six important consumer rights including safety, information, choice, redressal, and consumer education. These rights are protected by the Consumer Protection Act and help consumers get justice when faced with unfair trade practices.");
            break;

        case 'consumerresponsibilities':
            speak("Taking the Consumer Movement Forward: India observes December 24 as National Consumers' Day since the 1986 Consumer Protection Act.");
            break;
        ...
        case 'activities':
            speak("Test your knowledge and understanding with these interactive activities about Consumer Rights.");
            break;
...
function startJourney() {
    ...
    showAchievement('Journey Begins', 'You started your learning journey with Consumer Rights!');
    speak("Welcome to the interactive lesson on Consumer Rights. Let's begin by understanding consumer rights and how they protect us in our daily lives.");
}
...
function toggleDesc(button) {
    ...
    if (isCurrentlyExpanded) {
        ...
        // Stop any active narration for Consumer Rights cards specifically
        const introSpeakerBtn = card.querySelector('.intro-speaker-btn');
        if (introSpeakerBtn && introSpeakerBtn.classList.contains('active')) {
            introSpeakerBtn.classList.remove('active');
            stopAllNarration();
        }
...
function makeCardsClickable() {
    ...
    cards.forEach(card => {
        card.addEventListener('click', function(event) {
            ...
            if (event.target.closest('.intro-speaker-btn') ||
                event.target.closest('.intro-speaker-container')) {
                event.stopPropagation(); // Stop event propagation for narration buttons
...
function checkQuizAnswers() {
    ...
    // Question 1: b) Consumer Protection Act 1986
    const q1Answer = document.querySelector('input[name="q1"]:checked');
    if (q1Answer && q1Answer.value === 'b') {
        score++;
        results.push('<p class="correct-answer">✓ Question 1: Correct! The Consumer Protection Act was enacted in 1986 to protect consumer rights in India.</p>');
    } else {
        results.push('<p class="wrong-answer">✗ Question 1: Incorrect. The correct answer is b) Consumer Protection Act 1986.</p>');
    }

    // Question 2: b) Consumer courts
    const q2Answer = document.querySelector('input[name="q2"]:checked');
    if (q2Answer && q2Answer.value === 'b') {
        score++;
        results.push('<p class="correct-answer">✓ Question 2: Correct! Consumer forums and courts help consumers file complaints against unfair practices.</p>');
    } else {
        results.push('<p class="wrong-answer">✗ Question 2: Incorrect. The correct answer is b) Consumer courts.</p>');
    }
    ...
    if (q3Answer && q3Answer.value === 'd') {
        score++;
        results.push('<p class="correct-answer">✓ Question 3: Correct! Consumer responsibilities include being aware, choosing carefully, and speaking out against exploitation.</p>');
    } else {
        results.push('<p class="wrong-answer">✗ Question 3: Incorrect. The correct answer is d) All of the above.</p>');
    }
    ...
    if (score === totalQuestions) {
        setTimeout(() => {
            showAchievement('Quiz Master', 'Perfect score! You have excellent knowledge of Consumer Rights.');
        }, 1000);
    } else if (score >= 1) {
        setTimeout(() => {
            showAchievement('Good Understanding', 'Well done! You have a good grasp of Consumer Rights concepts.');
        }, 1000);
    }
}
...
function toggleIntroNarration() {
    ...
    const curatedContent = contentElement ? contentElement.textContent.trim() : "Globalisation is a process that has brought countries closer through trade, investment, and the growing influence of multinational corporations (MNCs). Over the past few decades, MNCs have played a central role in linking distant regions of the world, shaping production, markets, and economies. This chapter explores how globalisation works, the factors that drive it—such as technology, liberalisation, and international organisations—and its impact on development. It also encourages us to think critically about the opportunities and challenges globalisation creates for people, businesses, and nations.";
...
// ====== Consumer Rights Card Narration Functions ======

// Main Consumer Rights section narration
function toggleConsumerRightsNarration() {
    const btn = document.getElementById('consumerRightsSpeakerBtn');
    if (!btn) return;
    ...
    // Use curated content for Consumer Rights overview
    const curatedContent = "Consumer Rights are essential protections that help ensure fair treatment in the marketplace. This section covers six important consumer rights including safety, information, choice, redressal, and consumer education. These rights are protected by the Consumer Protection Act and help consumers get justice when faced with unfair trade practices.";
...
// Card 1: Safety is Everyone's Right
function toggleCard1Narration() {
    const btn = document.getElementById('card1SpeakerBtn');
    if (!btn) return;
...
// Card 2: Information about goods and services
function toggleCard2Narration() {
    const btn = document.getElementById('card2SpeakerBtn');
    if (!btn) return;
...
// Card 3: When choice is denied
function toggleCard3Narration() {
    const btn = document.getElementById('card3SpeakerBtn');
    if (!btn) return;
...
// Card 4: Where should consumers go to get justice?
function toggleCard4Narration() {
    const btn = document.getElementById('card4SpeakerBtn');
    if (!btn) return;
...
// Card 5: Learning to become Well-Informed Consumers
function toggleCard5Narration() {
    const btn = document.getElementById('card5SpeakerBtn');
    if (!btn) return;
...
// Card 6: ISI and Agmark
function toggleCard6Narration() {
    const btn = document.getElementById('card6SpeakerBtn');
    if (!btn) return;
...
function toggleMarketplaceNarration() {
    const btn = document.getElementById('marketplaceSpeakerBtn');
    if (!btn) return;
    ...
    // Get all paragraphs from the Marketplace section in HTML
    const contentElements = card.querySelectorAll('.content-block p');
    const curatedContent = contentElements.length > 0
        ? Array.from(contentElements).map(el => el.textContent.trim()).join(' ')
...
// Consumer Movement
function toggleConsumerMovementNarration() {
    const btn = document.getElementById('consumerMovementSpeakerBtn');
    if (!btn) return;
...
function toggleConsumerResponsibilitiesNarration() {
    const btn = document.getElementById('consumerResponsibilitiesSpeakerBtn');
    if (!btn) return;
